 ...
 * result set size exceeds the per page requestLimit.
 */
@SuppressWarnings({"UnusedReturnValue", "unused"})
public class PageLinksUtils {
     ...
     */
    public PageLinksUtils(Headers response) {
        String linkHeader = response.get(HEADER_LINK);
        if (linkHeader != null) {
            ...
            next = response.get(HEADER_NEXT);
            last = response.get(HEADER_LAST);
        }
    }
